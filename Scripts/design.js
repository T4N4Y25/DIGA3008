...
let arrSections = [ //An array storing names matching the data attribute of the links and the relevant section related to those links
...
arrLinks.forEach((link) => { //Iterate through the array with all the links
  let linkName = ""; //a variable to store the data attribute of the selected link
  link.addEventListener("click", function (e) { //Perfroms the subsequent code when the link is clicked, the e in the function declaration is to disable the auto-scrolling I initially had in HTML
    ...
    window.scrollTo({ //When a link is clicked, the page auto-scrolls to the top so the user can read from the beginning of each section
      top: 100,
      behavior: "smooth",
    });
    linkName = link.dataset.filter; //variable stores the data attribute of the clicked link
    arrSections.forEach((elem) => { //a nested loop to now iterate the array with all the sections and compare them with the stored data attribute
      if (linkName === "") { //Incase of errors
        return;
      }
      if (elem.name === linkName) { //Compare the elements name property with the data attribute of the clicked link
        // alert(elem.name); 
...
function SetColour(arr) { //Function to set the colour of the links to their default colour
  arr.forEach((colour) => {
    colour.classList.toggle("highlight", false);
  });
}

function Initialize(){ //Function to set the default section shown when the page loads
